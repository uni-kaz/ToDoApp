package jp.kobespiral.unikaz.todo.configuration;
...
@Configuration
@EnableWebSecurity // Spring Security を使うための設定
public class ToDoAppSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        // 認可の設定
        http.authorizeRequests().antMatchers("/sign_in").permitAll() // サインインページは誰でも許可
                .antMatchers("/sign_up/**").permitAll() // サインアップページは誰でも許可
    ...
    /**
     * アプリで共有のパスワード生成器を作る．Beanを付けているので任意の箇所でAutowired可能になる．
     */
    @Bean
